/* PQ navsd DistanceToNextManeuver shadow (fid 18): distance from NavState.distanceMeters via the stock formatter when AA active. */
...
extends DistanceToNextManeuverStage {
    ...
    private static final int WUSCHKE_DISTANCE_KM_CONSTANT = 0;
    private static final int WUSCHKE_DISTANCE_MILES_CONSTANT = 0;
    private static final int INVALID_DISTANCE = 0;
    ...
    private void setDistanceToNextManeuverStatusData(DistanceToNextManeuver_Status distanceToNextManeuver_Status) {
        if (NavState.ACTIVE) {   // AA active -> NavState.distanceMeters, bargraph off; else stock
            distanceToNextManeuver_Status.bargraphInfo.bargraphOnOff = 0;
            distanceToNextManeuver_Status.bargraphInfo.bargraph = 0;
            int dist = NavState.distanceMeters;
            if (dist > 0) {
                int[] nArray = new int[2];
                int unit;
                try { unit = this.getCurrentDistanceUnit(); } catch (Throwable t) { unit = 0; }
                switch (unit) {
                    case 1: {
                        ServiceManager.serviceManager.fixFormat.cnv2Distance2withMinDistanceMiles(dist, nArray);
                        break;
                    }
                    default: {
                        ServiceManager.serviceManager.fixFormat.cnvDistance2KilometersExt(dist, nArray);
                    }
                }
    ...
    private void sendDistanceToNextManeuverStatus(DistanceToNextManeuver_Status distanceToNextManeuver_Status) {
        this.getDelegate().getPropertyListener((BAPFunctionController)this).statusProperty((BAPEntity)distanceToNextManeuver_Status, (Property)this);
    }
